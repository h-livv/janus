...
from transport.analysis.metrics import compute_transport_metrics, write_metrics
from transport.io import SeedArrays, load_geant4_seeds
from transport.provenance import build_run_provenance, make_run_id, write_run_provenance
...
def run(
    *,
    ...
    if write_npz:
        ...
        metrics = compute_transport_metrics(result, output_name)
        metrics_path = write_metrics(metrics, run_outputs_dir)

        run_id = make_run_id(run_outputs_dir)
        provenance = build_run_provenance(
            ...
            output_artifacts={
                "transported_particles_npz": output_path,
            },
            metrics_path=metrics_path,
        )
        provenance_path = write_run_provenance(provenance, run_outputs_dir)

        from transport.analysis import analyze

        diagnostics = analyze(output_path, metrics=metrics)
        diagnostics["provenance"] = provenance_path
        print(
            f"[Analysis] Wrote diagnostics: "
            f"{', '.join(Path(p).name for p in diagnostics.values())}"
        )
