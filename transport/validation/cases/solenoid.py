"""Solenoid validation case stub (Level 1)."""
...
from transport.validation.references.analytical import StubAnalyticalReference
from transport.validation.sources.single import SingleParticleSource


def build_solenoid_case(
    ...
    return ValidationCase(
        ...
        references=[StubAnalyticalReference("solenoid_analytical")],
        metric_specs=[
            (MomentumConservationMetric(), Tolerance(1e-6)),
            (EnergyConservationMetric(), Tolerance(1e-6)),
        ],
        output_config=OutputConfig(),
        metadata={"element": "solenoid", "stub": True},
    )
