...
DetectorConstruction::DetectorConstruction() {
  ...
  fChamberWidth = 40.0 * cm;
  fChamberLength = 120.0 * cm;
  fTargetMaterialName = "G4_Ir";
  fTargetShape = "Cylinder";
  fTargetWidth = 3.0 * mm;
  fTargetLength = 55.0 * mm;
  fTargetPosition = G4ThreeVector(0., 0., -27.5 * mm);
...
G4VPhysicalVolume *DetectorConstruction::Construct() {
  ...
  if (!world_mat || !chamber_mat || !target_mat) {
    G4Exception("DetectorConstruction::Construct()", "Janus002", FatalException,
                "A specified material was not found in NIST database.");
  }

  // World
  G4double chamber_sizeXY = fChamberWidth;
  G4double chamber_sizeZ = fChamberLength;
  G4double world_sizeXY = 1.2 * chamber_sizeXY;
  G4double world_sizeZ = 1.2 * chamber_sizeZ;

  auto solidWorld = new G4Box("World", 0.5 * world_sizeXY, 0.5 * world_sizeXY,
                              0.5 * world_sizeZ);
  ...
  // Chamber
  auto solidChamber = new G4Box("Chamber", 0.5 * chamber_sizeXY,
                                0.5 * chamber_sizeXY, 0.5 * chamber_sizeZ);
  auto logicChamber = new G4LogicalVolume(solidChamber, chamber_mat, "Chamber");
  new G4PVPlacement(nullptr, G4ThreeVector(), logicChamber, "Chamber",
                    logicWorld, false, 0, checkOverlaps);

  // Target Shape Generation
  G4VSolid *solidTar = nullptr;
  G4double halfWidth = fTargetWidth / 2.0;
  G4double halfLength = fTargetLength / 2.0;

  if (fTargetShape == "Box") {
    solidTar = new G4Box("Target", halfWidth, halfWidth, halfLength);
  } else if (fTargetShape == "Cylinder") {
    solidTar = new G4Tubs("Target", 0., halfWidth, halfLength, 0., 360. * deg);
  } else if (fTargetShape == "Sphere") {
    solidTar =
        new G4Sphere("Target", 0., halfWidth, 0., 360. * deg, 0., 180. * deg);
  } else {
  ...
  new G4PVPlacement(nullptr, fTargetPosition, logicTar, "Target", logicChamber,
