...
4. `run` tracks and writes `transported_particles.npz` + `topology.json` + plots
5. Inherit once, mutate topology, construct + run again (study pattern)
6. `aperture_diameter` inserts circular `LimitEllipse` limits; particles outside are lost
...
- Transported NPZ schema, `topology.json`, and diagnostic PNGs
- Circular aperture construction and lost-particle `state`
