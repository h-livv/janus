...
Geant4 writes two ROOT files per run (staged under `temp/`, then packaged into `data/collision/<run_name>/`):
...
# or: python collision/validation/validate.py path/to/validation.root

# Phase 4: phenomenological plots (needs both ROOT files)
python collision/validation/physical_validation.py
# or: python collision/validation/physical_validation.py path/to/validation.root path/to/simulation.root
```

If paths are omitted, both scripts pick the newest directory under `data/collision/run_*`.
...
```text
collision/validation/validation_outputs/<run_name>/
├── phase4_pT_vs_pL.png
