...
* Target production (Geant4: `engines/geant4/` + `collision/`)
* Collision-stage validation (`collision/validation/`)
* Inherit of Geant4 ROOT `Seeds` (`transport/io.py`)
* Xsuite-backed drift, quadrupole, and bend transport via `Transport` + `config.json`
* Circular aperture (`aperture_diameter`) via Xsuite `LimitEllipse`
* Post-run diagnostic plots (`transport/plots.py`)
