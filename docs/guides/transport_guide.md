...
| `python transport/run.py` | At least one `data/interactions/*/simulation.root` ([collision guide](collision_guide.md)) |
...
| 4–5. Track + write | `run()` | convert, `line.track`, NPZ + `topology.json` |
...
| `source` | Path to `simulation.root`; `null` = newest under `data/interactions/` |
...
After `run()`, `t.particles` is the in-memory `xpart.Particles` object. Future studies compute observables from that or from the NPZ. Plots and metrics are not written by `run()`.

---

## Future studies (not a Janus package)

A sweep is a loop, the same idea as [interactions/run_batches.py](../../interactions/run_batches.py):
