...
The executable is `engines/geant4/build/janus`. Collision runs invoke it via `collision/interface.py`.
...
(`particle` is required by `collision/validation/validate.py` and particle-summary generation.)
