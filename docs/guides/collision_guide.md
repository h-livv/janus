...
Orchestration lives in `collision/interface.py`. It launches `engines/geant4/build/janus`, then packages ROOT outputs.

For headless / batch runs, set `"interactive": false` in `collision/config.json` (see below).

---

## Configuration

All collision scientific and run parameters live in `collision/config.json`.
...
Edit the JSON, then re-run `python collision/run.py`. Defaults in code also exist inside `Simulation` if the file is missing, but production studies should use the config file.
...
```bash
python collision/run_batches.py
```
...
If paths are omitted, scripts pick the newest `data/collision/run_*` directory.
...
Once `data/collision/*/simulation.root` exists:
